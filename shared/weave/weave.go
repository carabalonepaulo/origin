...
func (t *ChainedTask[T]) Add(thread byte, fn func(*T)) (*ChainedTask[T], error) {
	if t.idx > 0 {
		return nil, errors.New("can't chain after dispatched")
	}
...
func NewWorkerPool(cap int) *WorkerPool {
	w := &WorkerPool{
		tasks:    make([]Task, cap),
		dispatch: make(chan int, cap),
		resolve:  make(chan taskReport, cap),
	}
